...
void idle_on_exit(fsm_t *fsm, void *context) {
    printf("[idle] Exit!\n");
}

void walk_on_enter(fsm_t *fsm, void *context) {
    printf("[walk] Enter!\n");
}
...
void walk_on_exit(fsm_t *fsm, void *context) {
    printf("[walk] Exit!\n");
}
...
int main() {
    ...
    fsm_add_transition(g_fsm, "Idle", "Walk",
                       FSM_PREDICATE_GROUP(
                           transition_idle_to_walk));

    fsm_add_transition(g_fsm, "Walk", "Idle",
                       FSM_PREDICATE_GROUP(
                           transition_walk_to_idle));
